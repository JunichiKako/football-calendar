...
ICSでは同じUIDのイベントを更新するとき `SEQUENCE` を増やす必要があり、**前回配った番号を知らないと採番できません**。キャッシュは消えてよいものなので、この用途には使えません。

取得スクリプトが前回のスナップショットと突き合わせて採番し、JSON自身に持たせています。
...
ICS配信は `generateStaticParams` で事前生成しているため静的ルートになり、`Cache-Control: s-maxage=31536000` が付いてCDNがキャッシュします。**購読者が増えてもサーバーは処理しません**。

複数リーグの組み合わせは 2<sup>6</sup>-1 = 63通りあるため全部は焼かず、要求されたものだけ生成して以後キャッシュさせています。順序違いのURLが別キャッシュにならないよう、slugは定義順に正規化します。

### 画面外のカードを描画しない

全日程を表示すると試合カードが1,700枚を超え、DOMは約37,000要素になります。転送量は gzip 後 213KB で問題にならず、**効いているのは要素数**でした。

仮想化ライブラリを入れる前に、CSSで解けないかを試しています。
...
`content-visibility: auto` で画面外のカードはレイアウトと描画がスキップされます。`contain-intrinsic-size` で高さを予約しておかないとスクロールバーが伸縮するため、実測のカード高さを指定しています。
