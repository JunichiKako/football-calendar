          ...
          {dummyMatchList.map((league) => {
            return (
              <div key={league.id} className="flex flex-col items-center">
                <Image src={league.img} width={100} height={60} alt="" />
                <MatchCard date={league.date} matches={league.matches} />
              </div>
            );
          })}
        </div>
          ...
          {dummyMatchList.map((league) => (
            <div key={league.id} className="lg:hidden mt-6">
              <div className="flex items-end justify-between">
                <Image src={league.img} width={80} height={40} alt={league.league} />
              </div>
